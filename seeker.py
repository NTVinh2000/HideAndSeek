...
class Seeker:
    ...
    def findAnnounce(self):
        ...
        for i in range(0, len(self.vision)):
            for j in range(0, len(self.vision[i])):
                if self.vision[i][j] == 5:
                    temp = []
                    #hider_x = self.Sx - (3 - i)
                    #hider_y = self.Sy - (3 - j)
                    announce_x = self.Sx + (i-vision_x)
                    announce_y = self.Sy + (j-vision_y)
                    temp.append(announce_x)
                    temp.append(announce_y)
                    flag = False
                    for k in self.announcePositionList:
                        if k[0] == announce_x and k[1] == announce_y:
                            flag = True
                            break
                    if flag == False:
                        self.announcePositionList.append(temp)

        return self.announcePositionList

    def findAnnounceArea(self,map,cor):
        temp = cor
        for i in range(temp[0]-3,temp[0]+3):
            for j in range(temp[1]-3,temp[1]+3):
                if i >= 1 and i <= len(map)-1  and j >=1 and j <= len(map[0])-1 :
                    #if map[i][j] ==0  or map[i][j] == 5 or map[i][j] ==2 :
                    if map[i][j] != 1:
                        self.announceArea.append([i,j])
        return  self.announceArea



    def randomHeuristic2(self, currentTime, visitMap, announceZone=[]):
        '''
        count = 0
        for row in self.vision:
            count += row.count(VISIBLE)
        ...
        for row in range(self.top, self.bottom + 1):
            for col in range(self.left, self.right + 1):
                if self.vision[row - self.top][col - self.left] != VISIBLE:
                    continue
                count += (currentTime - visitMap[row][col])**2
                if (visitMap[row][col] == 0):
                    zero_cnt += 1
                if isAnnounce and ([row, col] in announceZone):
                    count += 100

        return (count, zero_cnt)
    def heuristic(self,cor, map, currentTime, visitMap,Azone):
        dummy = Seeker()
        dummy.Sx = cor[0]
        dummy.Sy = cor[1]
        dummy.visionScopeUpdate(map)
        ...
        return dummy.randomHeuristic2(currentTime, visitMap,Azone)

    def findOptimalPoint(self,map):

        max = 0
        optimal_point = []
        temp_x = 0;
        temp_y = 0;
        for i in self.announceArea:
            try:
                ...
                print( self.Sx," ",self.Sy)
                print(i)
            optimal_point.append(temp_x)
            optimal_point.append(temp_y)
        return optimal_point
    def updateAnnounceArea(self):

        for i in range(0,len(self.announceArea)):
            try:
                if self.valueInVision(self.announceArea[i][0],self.announceArea[i][1]) == 0:
                    self.announceArea.pop(i)
                if self.announceArea[i][0] == self.Sx and self.announceArea[i][1] == self.Sy:
                    self.announceArea.pop(i)
            except:
                print('')
                continue
